...
function fillPlayer(player,nr, side, max){
    ...
    $bar2.find(".knife").removeClass("dead").addClass(statistics.health == 0 ? "dead" : "");

    $top.find("#hp_p").text(statistics.health);
    $top.find(".hp_bar").css("background", hpbar);
    $bottom.find(".equip_bar").css("background", background_equip_bar);
    $bar2.css("background", background_equip_bar);

    $bar2.find(".kills").text(statistics.kills+"/");
    $bar2.find(".assists").text(statistics.assists+"/");
    $bar2.find(".deaths").text(statistics.deaths+"");

    $bottom.find(".hp_el").html(statistics.helmet ? $("<img />").attr("src", "/files/img/helmet_new.png") : statistics.armor > 0 ? $("<img />").attr("src", "/files/img/armor_new.png") : "");
    ...
    for(let key in weapons){
        ...
        if(type != "C4"){
            view += weapon.state == "active" ? " checked " : " knifefilter ";
            if(type == "Grenade"){
                for(let x = 0; x < weapon.ammo_reserve; x++){
                ...
                if(type == "Knife") {
                    $bar2.find(".knife").html($("<img />").attr("src", "/files/img/weapons/knifes/" + name + ".png.png").addClass(view));
                } else if (type == "Pistol") {
...
function updatePage(data) {
    ...
    $("#round_counter").html("Round " + round_now);
    //TEAMS
